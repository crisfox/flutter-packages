...
import 'button_configuration.dart' show ButtonConfiguration, convertButtonConfiguration;
import 'dom.dart';
...
class GisSdkClient {
  ...
  GisSdkClient({
    ...
    String? hostedDomain,
  }) : _initialScopes = initialScopes, _loggingEnabled = loggingEnabled {
    if (_loggingEnabled) {
  ...
  void _logIfEnabled(String message, [List<Object?>? more]) {
    if (_loggingEnabled) {
      domConsole.log('[google_sign_in_web] $message', more);
    }
  }
...
//TODO(dit):This is not good enough, we need to be able to send null userDataEvents too!

    userDataEvents = _credentialResponses.stream
      .map(utils.gisResponsesToUserData)
      .handleError((Object error) {
        _logIfEnabled('Removing error from `userDataEvents`:',  <Object>[error.toString()]);
      });
  }
  ...
  Future<void> renderButton (Object parent, ButtonConfiguration? options) async {
